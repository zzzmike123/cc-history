...
def api_resume():
    ...
    real_path = session.get("realProjectPath") or str(PROJECTS_DIR)
    cwd = Path(real_path).expanduser()
    if not cwd.exists():
        return _json_error(f"项目路径不存在：{real_path}", 404)

    try:
        script = "Set-Location -LiteralPath $args[0]; claude --resume $args[1]"
        creationflags = getattr(subprocess, "CREATE_NEW_CONSOLE", 0)
        subprocess.Popen(
            ["pwsh", "-NoExit", "-Command", script, str(cwd), session_id],
            creationflags=creationflags,
        )
        return jsonify({"ok": True, "message": "已启动"})
    except FileNotFoundError:
        return _json_error("未找到 pwsh，请确认 PowerShell 7 已安装并在 PATH 中", 500)
    except Exception as exc:
